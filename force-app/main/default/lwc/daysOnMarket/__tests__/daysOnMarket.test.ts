// noinspection MagicNumberJS,LocalVariableNamingConventionJS,SpellCheckingInspection
...
import LightningFormattedDateTime from "lightning/formattedDateTime";
...
describe('c-days-on-market', () => {
    ...
    it('registers the LMS subscriber during the component lifecycle', () => {
        ...
        // Validate if pub/sub got registered after connected to the DOM
        expect(subscribe).toHaveBeenCalled();
        expect((<jest.MockInstance<any, any>><unknown>subscribe).mock.calls[0][1]).toBe(PROPERTYSELECTEDMC);
    });
    ...
    it('unregisters the LMS subscriber during the component lifecycle', () => {
        ...
        // Validate if pub/sub got unsubscribed after disconnected from the DOM
    ...
    it('invokes getRecord with the published message payload value', async () => {
        ...
        // Simulate publishing a message using RECORD_SELECTED_CHANNEL message channel
        const messagePayload = <DaysOnMarket>{ propertyId: '001' };
        ...
        const { propertyId, fields } = (<LdsTestWireAdapter><unknown>getRecord).getLastConfig();
        expect(propertyId).toEqual(messagePayload.recordId);
        expect(fields).toEqual([DATE_LISTED_FIELD, DAYS_ON_MARKET_FIELD]);
    });

    describe('getRecord @wire data', () => {
        function validateHTML(element: DaysOnMarket, type: string) {
            ...
            const chartBarEl = element.shadowRoot.querySelector<HTMLElement>(
                `div.bar.${type}`
            );
            ...
            const formattedDateTimeEl = element.shadowRoot.querySelector<LightningFormattedDateTime>(
