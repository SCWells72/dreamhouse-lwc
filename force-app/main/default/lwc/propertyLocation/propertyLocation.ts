...
import { getLocationService, LocationResult, LocationService} from 'lightning/mobileCapabilities';
...
export default class PropertyLocation extends LightningElement {
    ...
    distance: number;
    location: LocationResult | GeolocationPosition;
    @api recordId: string;
    ...
    calculateDistance() {
        if (this.location && this.property) {
            const latitude1: number = this.location.coords.latitude;
            const latitude2: number = Number(getFieldValue(this.property, LATITUDE_FIELD));
            const longitude1: number = this.location.coords.longitude;
            const longitude2: number = Number(getFieldValue(this.property, LONGITUDE_FIELD));
